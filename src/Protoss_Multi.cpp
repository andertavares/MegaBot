		...
		if(build_frame == Broodwar->getFrameCount())
		{
			...
			if(Broodwar->enemy()->getRace() == Races::Terran || Broodwar->enemy()->getRace() == Races::Protoss)
			{
				...
				if(Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Gateway)>=3)
				{
					...
					this->buildOrderManager->build(1,UnitTypes::Protoss_Assimilator,772); //가스						
					...
					gas_carrying_probe_num++;
						...
						*/
							...
							if(aaab == true) //안드로메다
							{
								...
								if(Broodwar->self()->gas() > 200)
									this->buildOrderManager->build((int)(BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Zealot)*0.7),UnitTypes::Protoss_Dragoon,720); //드라군은 질럿의 0.7배 뽑기
							}
							else
							{
									if(Broodwar->self()->gas() > 200)
							...
							if(aaab == false)
							{
										if(BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Dragoon) >= 3) //드라군이 3마리 이상이면 가스 2개 한번에 지음
							{
								this->buildOrderManager->build(2,UnitTypes::Protoss_Assimilator,826);
							}
							...
							if(BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Protoss_Dragoon) >= 7) //드라군이 7마리 이상이면  로보틱스, 옵저버터리, 아둔, 템플러 아카이브 지음
							{
								//this->buildOrderManager->build(1,UnitTypes::Protoss_Assimilator,822);
